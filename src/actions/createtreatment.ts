...
export const createtreatment = async (treatmentData : treatmentDataProps, metamaskAddress : string) => {

  if(metamaskAddress) {
    await db.treatment.create({
      data: {
        patientAddress: treatmentData.patientAddress,
        guardianAddress: treatmentData.guardianAddress,
        doctorAddress: metamaskAddress,
        treatmentTitle: treatmentData.treatmentTitle,
        treatmentDetails: treatmentData.treatmentDetails,
        treatmentCourse: treatmentData.treatmentCourse,
      },
    });
    return { success: 'Record successfully created'};
  } else {
    return { error : 'no metamask Address found'};
  }
